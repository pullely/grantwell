...
// deleted; all wire I/O now goes through the typed `Grantwell` resource
...
//   - `createClient` — constructs a `Grantwell` against a target, wired
...
//     `Promise<T>` + thrown `GrantwellError` into the same shape the
...
import { Grantwell, GrantwellError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Grantwell;
...
export function createClient(target: ApiTarget, token: string | null): Grantwell {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Grantwell(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `GrantwellError` subclasses surface as `{ ok: false }` carrying the
...
export async function wrap<T>(fn: () => Promise<T>): Promise<ApiResult<T>> {
  try {
    ...
    if (e instanceof GrantwellError) {
