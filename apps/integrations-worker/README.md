...
Your backend holds one Grantwell API key and **zero GitHub credentials**.
...
const grantwell = new Grantwell({
  baseUrl: process.env.GRANTWELL_API_URL!,
  token: process.env.GRANTWELL_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await grantwell.integrations.issueGithubToken(ORG_ID, {
