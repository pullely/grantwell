...
describe("resolveConfigDir", () => {
  it("respects GRANTWELL_CONFIG_DIR override", () => {
    const prev = process.env["GRANTWELL_CONFIG_DIR"];
    process.env["GRANTWELL_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["GRANTWELL_CONFIG_DIR"];
      else process.env["GRANTWELL_CONFIG_DIR"] = prev;
    }
  });
});
