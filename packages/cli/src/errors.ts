// CLI error helpers. Translate `GrantwellError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `grantwell org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `grantwell login` to refresh", mode);
  }
  if (err instanceof GrantwellError) {
