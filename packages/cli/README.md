...
`grantwell` — first-class TypeScript CLI for the Grantwell control
...
This package is internal to the `grantwell` monorepo and is not
...
```
grantwell login    [--api-url=URL] [--token=BEARER]
grantwell logout
grantwell whoami
```
...
```
grantwell org list
grantwell org use <org-id>
grantwell org members
grantwell project list
```

Writes (Task 0101):

```
grantwell org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
grantwell project create <name> [--idempotency-key=KEY]
grantwell env create <project-id> <name> [--idempotency-key=KEY]
grantwell api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
grantwell webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
grantwell usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
grantwell billing summary
grantwell audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`grantwell org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `grantwell login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/grantwell/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/grantwell/config.json` (mode 0644, not a secret). Override
both via `GRANTWELL_CONFIG_DIR` (used by tests).
