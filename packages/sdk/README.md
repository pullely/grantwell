...
Grantwell TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Grantwell control plane API.

## Install (workspace)

This package is internal to the `grantwell` monorepo. Add it as a
...
```ts
import { Grantwell } from "@saas/sdk";

const client = new Grantwell({
  baseUrl: "https://api.grantwell.app",
  auth: { kind: "bearer", token: process.env["GRANTWELL_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `GrantwellError`. Branch on
...
| `GrantwellError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `GrantwellError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Grantwell({ fetch })`.
