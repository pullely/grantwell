...
import { Grantwell } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Grantwell({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => GrantwellError]> = [
  ...
  it("forward-compat: unknown error code → base GrantwellError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(GrantwellError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as GrantwellError).code).toBe("future_quota_exceeded");
    }
  });
