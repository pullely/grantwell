// Base HTTP transport for the Grantwell SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `GrantwellError`
...
export interface ClientOptions {
  /**
   * Base URL of the Grantwell api-edge worker, e.g.
   * `https://api.grantwell.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Grantwell` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Grantwell SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Grantwell SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
